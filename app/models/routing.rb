class Routing < ActiveRecord::Base
  ...
  def standby_passengers
    midnight = Passenger.midnight_local_to_utc
    end_of_day = Passenger.end_of_day_local_to_utc
    pax_list = passengers.joins(:priority).where("accepted IS NOT TRUE AND passengers.created_at >= ? and passengers.created_at <= ?", midnight, end_of_day).order('pri_level asc, sequence asc')
    pax_list
  end
